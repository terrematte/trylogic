...
let concl_replace s =
  ...
  let s = r "False" "⊥" s in
  let s = r "True" "\\u22a4" s in
  let s = r "v[|][|]--" "<font color=\"green\">v||--</font>" s in
  let s = r "v[|][|]-/-" "<font color=\"red\">v||-/-</font>" s in  
  hannos_space s
;;

let mylength s =
  let s2 = Pcre.substitute
      ~rex:(Pcre.regexp "<sup>|</sup>|<font color=\"green\">|<font color=\"green\" size=\"5\">|<font color=\"red\" size=\"5\">|<font color=\"red\">|<pre>|</pre>|</font>|[&]lt|\\\\u[0-9a-f][0-9a-f][0-9a-f]")
      ~subst:(fun _ -> "") s
  in
  String.length s2
;;
